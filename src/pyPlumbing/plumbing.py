...
class Plumbing:
    ...
    def __init__(self, vertices_dict, edges, edge_signs=None):
        try:
            ...
            self._edge_list = [
                (self._vertex_list[x[Integer(0)]], self._vertex_list[x[Integer(1)]]) for x in edges
            ]

            self._graph.add_vertices(self._vertex_list)
            self._graph.add_edges(self._edge_list)
            self._plot_options = {"vertex_color": "black", "vertex_size": Integer(20)}
            if self._graph.is_tree():
                self._plot_options["layout"] = "tree"

            if edge_signs == None:
                edge_signs = [Integer(0)] * self._edge_count
            else:
                assert (
    ...
    def from_plumbing_matrix(cls, plumbing_matrix):
        ...
        assert (
            len(set(list(map(len, plumbing_matrix)) + [len(plumbing_matrix)])) == Integer(1)
    ...
    def invert_orientation(self):
        return Plumbing(
            ...
            [(x[Integer(1)], x[Integer(0)]) for x in self._edges],
        )
    ...
    def plumbing_matrix(self):
        ...
        if self._plumbing_matrix is None:
            ...
            for edge, sign in zip(self._edges, self._edge_signs):
                i, j = edge
                if i != j:
                    plumbing_matrix[edge[Integer(0)], edge[Integer(1)]] += (-Integer(1)) ** (sign)
                    plumbing_matrix[edge[Integer(1)], edge[Integer(0)]] += (-Integer(1)) ** (sign)
                else:
                    plumbing_matrix[edge[Integer(0)], edge[Integer(1)]] += Integer(2) * (-Integer(1)) ** (sign)
        ...
        if self._is_Seifert is None:
            if self.is_tree == False:
                ...
                high_valency_node_count = sum(
                    Integer(1) for d in self._degree_vector if d[Integer(0)] > Integer(2)
                )
    ...
    def norm_fac_euler(self):
        ...
        if self._norm_fac_euler == None:
            ...
            factors = [
                (Integer(1) / (self.orbifold_euler * p)).denominator() for p in p_coefficients
            ]
    ...
    def eff_inv_euler(self):
        ...
        if self._eff_inv_euler == None:
            self._eff_inv_euler = (
                -self.plumbing_matrix_inverse[Integer(0), Integer(0)] * self.norm_fac_euler
            )
    ...
    def _zhat_prefactor(self, type_rank):
        ...
        return Series.from_symbolic(
            (-Integer(1))
            ** (Phi * pos_eigen) * q
            ** ((Integer(3) * sig - self.plumbing_matrix.trace()) / Integer(2) * norm_rho2)
        )
    ...
    def zhat(
        ...
        wilson=None,
        basis="weight",
        n_powers_start=Integer(1),
        div_factor=Integer(100),
        method="cython",
        info=False,
    ):
        """
        ...
        while max_power_computed <= order:
            ...
            exponent_products2, prefactor_products2 = self._ell_setup(
                type_rank, n_powers + Integer(1), wilson=wilson
            )
            # Remove terms that are already computed
            new_terms = np.logical_not(
                np.all(np.isin(exponent_products2, exponent_products), axis=(Integer(1), Integer(2)))
            )
            ...
            n_powers += (
                int((order - max_power_computed) / div_factor) + Integer(1)
            ...
            if degree == Integer(0):  # If the degree is zero, the contribution is just one
                node_contributions_exponents.append(
                    [
                        ...
                        for g1, g2 in itertools.product(WG, repeat=Integer(2))
                    ]
                )
                node_contributions_prefactors.append(
                    [l1 * l2 for l1, l2 in itertools.product(WL, repeat=Integer(2))]
                )
    ...
    def _compute_zhat(
        ...
        matrix_products = (
            ...
            @ (
                ...
                - (
                    ...
                    ).transpose(Integer(0), Integer(2), Integer(1))
                )[:, np.newaxis, :]
            )
            @ cartan_i
        )
        non_int_part, _ = np.modf(np.round(matrix_products, Integer(9)))
        condition = np.concatenate(np.all((np.abs(non_int_part) < RealNumber('1e-8')), axis=(Integer(2), Integer(3))))
        ...
        q_powers = [QQ(-Integer(1) / Integer(2) * t) for t in L_norms]
        series_numerical = [
            [tuple(p), Integer(1) / (Integer(2) * len(WG)) * c]
